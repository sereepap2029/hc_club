...
?>
    	...
    	<div class="container-fluid">
		  <div class="row">
		    <div class="col col-md-2">
		      <a href="<?=site_url("bang/create")?>" class="btn btn-success">Add</a>
		    </div>
		  ...
		  <div class="row">
		  	<div class="col col-md-12">
				  ...
				  <tbody>
				  	<?
				  	foreach ($bang_list as $key => $value) {
				  		$withdraw=$ci->m_bang->get_withdraw_by_bang_id($value->id);
				  		$numwith=count($withdraw);
				  		?>
				  		<tr>
					      <td scope="row"><?=($key+1)?></td>
					      <td><a href="<?=@site_url("bang/create/".$value->id)?>"><?=$value->borrower?></a></td>
					      ...
					      <td><?=$numwith?> ครั้ง</td>
					      <td>
				             <?/*<button type="button" class="btn btn-secondary" onclick="location.href='<?=@site_url("bang/create/".$value->id)?>';">แก้ไข</button>*/?>
				             <button type="button" class="btn btn-danger" onclick="deleteconfirm('<?=@site_url("bang/delete/".$value->id)?>')">ลบ</button>
				          </td>
					    </tr>
				  		<?
				  	}
    ...
    function sub_search(){
          ...
          $(myform).attr("action","<?=site_url("main/bang/")?>");   
